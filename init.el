...
(defun personal/set-top-level ()
  "Set top level form to the `i` register."
  (interactive)
  (save-excursion
    (beginning-of-defun)
    (let ((beg (point)))
      (clojure-forward-logical-sexp)
      (set-register ?i (buffer-substring beg (point)))
      (message "set i register to %s " (s-truncate 40 (buffer-substring beg (point)))))))
